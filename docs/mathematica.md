...
while you are in the appropriate mamba environment (see below).
...
[`mamba`](https://mamba.readthedocs.io/en/latest/index.html) (which is a drop-in
replacement for `conda`), you can run

```bash
mamba create -n sxs_mathematica -c conda-forge sxs pyzmq pyarrow
```

Note that this command just *creates* the environment; if you want to use it
directly, you need to run `mamba activate sxs_mathematica`, which basically
