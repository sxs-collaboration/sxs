...
from . import PNPsiMModes
from ...waveforms import WaveformModes
import numpy as np
import quaternionic

def PNWaveform(q,M,omega_0,chi1_0,chi2_0,frame_0=quaternionic.one,t_0=0.0, t_PNStart=False, t_PNEnd=False, datatype="h", return_chi=False, PNEvolutionOrder=4.0, PNWaveformModeOrder=3.5, TaylorTn=1, StepsPerOrbit=32, ForwardInTime=True, tol=1e-10, MinStep=1e-7):
    """
    ...
    PN=PNEvolution.PNEv.Evolution(wHat, xHat, yHat, zHat, m1, m2, v_0,S_chi1_0, S_chi2_0, frame_0.ndarray, t_PNStart, t_PNEnd,
    ...
    if datatype=="h":
        data, [ellmin,ellmax] = PNWaveformModes.Modes(wHat, xHat, yHat, zHat, m1, m2, v_0,S_chi1_0, S_chi2_0, frame_0.ndarray, PN.y, PNWaveformModeOrder)
        ...
        data, [ellmin,ellmax] = PNPsiMModes.Modes(wHat, xHat, yHat, zHat, m1, m2, v_0,S_chi1_0, S_chi2_0, frame_0.ndarray, PN.y, PNWaveformModeOrder)
        data[:,0]=data[:,0]-1.0
        W_PN_corot=WaveformModes(data,
            time=PN.t+t_0,
            frame=quaternionic.array(np.column_stack((PN.y[5],PN.y[6],PN.y[7],PN.y[8]))).normalized,
