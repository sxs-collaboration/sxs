"""Class encapsulating "deposit" objects on CaltechDATA
...
class Deposit(object):
    def __init__(self, login, deposition_id=None, ignore_deletion=False):
        ...
        updating, or replacing an entry in CaltechDATA.

        Parameters
        ----------
        login: Login object
            Object containing a `requests.Session` object that can successfully interact with the
            CaltechDATA API.  See the help string for `Login` in this module.
        ...
        if deposition_id is not None:
            ...
            url = "{0}api/records/{1}".format(self.base_url, deposition_id)
            r = self._get(url)
    ...
    def refresh_information(self):
        """Retrieve current information about this Deposit from CaltechDATA
    ...
    def files(self):
        ...
        Each file is described by a dictionary containing these keys:
            ...
            * filename
            * filesize (in bytes)
            * id (CaltechDATA-generated hex id of this file)
            * links
                * download
                * self
    ...
    def update_metadata(self, metadata, refresh_information=True):
        ...
        as desired, and then use this function to update it on CaltechDATA.
        ...
        access_conditions: string, required if 'access_right' is 'restricted'
            ...
            allowed to charge users for granting access to data hosted on CaltechDATA. Following HTML
        ...
        communities: list of dictionaries
            ...
            element is a dictionary with the key 'identifier' followed by the name of a CaltechDATA
            community.
    ...
    def upload_file(self, path, name=None, relpath_start=None, skip_checksum=False, refresh_information=True):
        ...
        The current list of files uploaded to CaltechDATA is checked.  If `name` is in that list, the MD5
        checksum of `path` is evaluated and compared to the MD5 checksum of the file on CaltechDATA.  If
        ...
        skip_checksum: bool [default: False]
            ...
            exactly this name and checksum has been uploaded to CaltechDATA; if so, don't bother
            uploading it again.
    ...
    def prereserve_doi(self, refresh_information=True):
        ...
        Note that this is undocumented behavior of the CaltechDATA API, so it may change without notice;
    ...
    def __del__(self):
        if not self.published and not self.ignore_deletion:
            ...
            warning = r"""
            The CaltechDATA Deposit object has not been published.  The deposit id is '{deposition_id}'.
            ...
            You can also perform these actions using this interface by running
                ...
                >>> from sxs.caltechdata import Login
