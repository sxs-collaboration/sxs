def minimal_grid(x, y, tol=1e-6, rel=False):
    ...
    from `romspline` -- namely, `romspline.greedy._greedy` --
    tailored to the needs of this package.  This function should
    behave nearly the same as the `romspline` function (except that
    this function returns fewer quantities), but also accepts a
    callable object as the `tol` parameter, to enable more general
    evaluations of the error.
    ...
    tol: float or callable [defaults to 1e-6]
        If given a float, this function will iteratively increase the
        number of sampling points until the spline formed from those
        points is within this value of the input data at each input
        sample.  If given a callable object, that object must be as
        described in the Notes section below.
    rel: bool [defaults to False]
        If `tol` is a float and this is true, the tolerance will be
    ...
    Notes
    =====
    The `tol` callable, if given, is used to decide whether or not
    the current trial spline is acceptable, and if not which sample
    point to include for the next trial spline.  It must be callable
    with the signature (x, y, y_greedy), and is expected to return
    either None (if the trial is acceptable) or the index of the next
    sample point to include.  Here, `x` and `y` are just the
    corresponding inputs to this function, and `y_greedy` is a set of
    data points representing the values of the trial reduced-order
    spline evaluated on `x`.  For example, the following function
    would behave equivalently to the default parameters:
